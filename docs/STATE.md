...
> 최종 갱신: 2026-08-17 — M9-A 방탈출 인터랙션 기반(사물 좌클릭→하단 설명) 완료. ⚠️이동 정지 버그 발견(아래)
...
1. **🔴 이동 정지 버그(신규, M9-A 검증 중 발견 — M9-A와 무관, stash 대조로 사전존재 확인)**: `m4-rooms.mjs` 8/13 실패 — 거실 통과 이동 중 특정 지점(책상/의자 콜라이더 모서리 추정)에서 위치 고정, 이후 전 구간 그 좌표로 멈춤(루프/입력은 정상, `resolve()` 보정만 안 됨 — corner-trap 의심, `physics/resolve.js` 검토 필요)
...
- 방 크기 6.0/5×4/4×5/4×4 m 경로는 **위 이동 정지 버그로 m4-rooms.mjs 13/13 재현 안 됨**(8/13, 위 항목 1 참고)
