...
> 최종 갱신: 2026-08-17 — resolve() MTV 재구조화 완료(진단·실측 기반). 거실 문D1 코너는 구조적 한계로 미해결(아래)
...
| **M4 집 4칸 구조** (`layout.js`+`house.js`, 거실 소품) | ✅ 완료 — `layout-check.mjs` 7/7. `m4-rooms.mjs`는 현재 8/13(열린 작업 1번 참고) |
...
1. **🟡 이동 정지 버그 — 원인 규명 완료, 부분 수정(resolve.js MTV화), 근본 케이스는 미해결**: `physics/resolve.js`를 "겹치는 콜라이더 전부 순차 적용"에서 "반복마다 가장 깊은 것 하나만 해소"(MTV 정석)로 바꿨다. 실측 결과 원인은 사용자가 추정한 "한 콜라이더의 반대 push 상쇄"가 아니라, **서로 무관한 콜라이더 2개**(문D1 동쪽 벽 잔여조각=X만 막음, `living.desk`=Z만 막음)가 각자 다른 축을 막아 대각선 이동을 정확히 상쇄하는 것이었다(간격 0.115m, 코너에 반지름만큼 정확히 접함). MTV로 바꿔도 이 특정 케이스는 **똑같이 재현됨**(실측 확인 — 두 콜라이더가 독립적이라 순서를 바꿔도 결국 둘 다 적용됨). `m4-rooms.mjs` 여전히 8/13, `tools/render-check/stuck-diagnose.mjs`가 회귀 검사로 상시 편입됨(1개 알려진 실패 유지, exit 1이 정상)
...
- 방 크기 6.0/5×4/4×5/4×4 m 경로는 **위 이동 정지 버그로 m4-rooms.mjs 13/13 재현 안 됨**(8/13, 위 항목 1 참고). 책상↔의자 실측 간격은 0.315m(플레이어 지름 0.44m보다 좁음 — 원래 통과 불가가 정상, 버그 아님)
