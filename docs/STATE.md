...
> 최종 갱신: 2026-08-16 (Claude Code CLI) — 걷는 캐릭터+소리+방4개 갖춰짐, M5는 M4c 드레싱 게이트만 남음
...
| M4c 방 3개 드레싱 | 🟡 `gemini/lane-rooms`에 새 커밋 있음(`e79a423`) — 아직 리뷰 전, **M5의 마지막 게이트** |
| M5 배포 (itch.io) | 🔴 M4c 완료 후 판단 — 그 외엔 배포 판단 가능한 상태(구조/이동/캐릭터/오디오 전부 ✅) |
...
| 브랜치 | 담당 | 상태 |
|---|---|---|
| `main` | 2층 CLI | M4 구조 + 색보정/조명 재작업 + 캐릭터 merge + 오디오 merge 전부 완료 |
| `gemini/lane-character` | 3층 | merge 완료(`6e68173`) — 삭제 대상(아직 안 지움) |
| `gemini/lane-audio-content` | 3층 | merge 완료(`4015c9e`) — 삭제 대상(아직 안 지움) |
...
1. **`gemini/lane-rooms` 리뷰·merge** — 2층, 새 커밋(`e79a423`) 도착함, **M5 배포의 마지막 게이트**
2. 벽 페이드 (`TAG.FADEABLE` 실제 투명도) — 2층, 문 있는 벽 구조가 이제 있으니 착수 가능
3. 카메라 프레이밍 재확인 — 로봇 캐릭터가 캡슐보다 크고 둥글어서 화면을 많이 채움(스크린샷 육안 확인, `dev-log.md` (20) 참고). 수치 조정 필요할 수 있음
